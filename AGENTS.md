...
| Target SDK | Android API | 34 | Play Store requirement (Aug 2024+) |
...
| Emulation | LibretroDroid | 0.6.2+ | AAR/JAR via Maven or local; cores fetched at build time |
...
| Persistence | JSON (files) + Room (optional) | — | BaseRomRepository: JSON in `filesDir`; PatchRepository: files in `cacheDir` |
| Coroutines | kotlinx-coroutines | 1.8+ | `lifecycle-runtime-ktx` for ViewModelScope |
| Reactive | RxJava/RxAndroid | 2.x | KEPT as-is: frozen `gamepad/` package depends on it (`CompositeDisposable`, `pad.events()`). New code (store, settings) uses coroutines/Flow; do NOT refactor gamepad to Flow |
...
│   ├── ui/              # SettingsActivity, Fragments (BaseRomImport, BaseRomList, CatalogUrl)
...
| **Bruce** 🦈 | **Primary Implementer** — all Kotlin/Android code (Phases 0–4) | All implementation tasks: patcher, store, settings, retroview, UI |
| **Dolfi** 🐬 | Icons/covers — generates SVG icons (app icon, hack category icons) and PNG cover placeholders for hacks without `coverImageUrl` | When UI needs icons or cover art |
...
| **Calamari** 🦑 | Fact-checking — validates known ROM checksums (No-Intro/Redump), verifies LibretroDroid/core versions, checks BPS spec details | When Bruce needs verified data |
| **Puffy** 🐡 | Research — up-to-date LibretroDroid releases, core buildbot URLs, Android API changes, Gradle plugin updates | When Bruce needs current docs |
| **Chululu** 🐙 | Visual QA — analyzes screenshots of Library/Store/Game UI for layout, alignment, accessibility | Before UI merges, release candidates |
...
| App icon, hack category icons, cover placeholders | **Dolfi** |
| README, strings translation, KDoc | **Wally** |
| ROM checksum verification, core version check | **Calamari** |
| LibretroDroid/core/Android API research | **Puffy** |
