...
object N64ControllerMapping {
    ...
    fun currentProfile(context: Context): Profile {
        ...
        val enabled =
                if (hasNewKey) {
                    ...
                    ButtonStickMode.values().getOrElse(modeIndex) { ButtonStickMode.C_RIGHT } !=
                            ButtonStickMode.OFF
                }
        val mode = if (enabled) ButtonStickMode.C_RIGHT else ButtonStickMode.OFF
        return Profile(
    ...
    data class Profile(val buttonStickMode: ButtonStickMode, val autoZEnabled: Boolean)

    /**
     * Resolves a raw physical key to the N64 control that [ControllerInput] sends to the core.
     * [autoTarget] is the latest C-button in Auto mode; game sessions and the tester both start it
     * at C-Right.
     */
    ...
    private fun ButtonStickMode.toN64Control(autoTarget: Control): Control? =
            when (this) {
