 ...
 *
 * Mirrors the proven logic in [br.com.redclaw.zelda64player.store.DownloadManager]:
 * for BPS it reads the patch's expected source CRC32 and matches it against the
 * user's imported base ROMs (reusing [findBaseRomByCrc]); for IPS the patch is
 * self-contained. On success the patched ROM is written to [Storage.rom] and the
 * hack is recorded in [UserHacksRepository] + [InstalledHacksRepository] so it
 * surfaces in the Library. On a missing base ROM it reports which game the patch
 * targets (via [KnownBaseRomTable]) instead of failing silently.
 */
class ImportedPatchInstaller(
            ...
            val baseRom: BaseRom? = if (format == PatcherFacade.PatchFormat.BPS) {
                ...
                if (found == null) {
                    ...
                    return@withContext ImportPatchNoCompatibleRom(
                        ...
                        foundCrc32s = baseRomRepository.getAll().map { rom -> rom.crc32 }
                    )
                }
                found
            } else {
                ...
                val entry = HackEntry(
                    ...
                    baseRom = baseRom?.let {
                        BaseRomRef(
                            ...
                            versionByte = it.versionByte,
                            checksums = Checksums(it.crc32, it.md5, it.sha1)
                        )
                    } ?: BaseRomRef(
                        name = context.getString(R.string.import_unknown_base),
                        gameCode = "",
                        versionByte = 0,
    ...
    private fun mapMessage(e: Throwable): String {
        val detail = when (e) {
            ...
            is PatcherException.RomFormatError ->
                e.message ?: "Invalid ROM format"
            else -> e.message ?: "Patch application failed"
        }
        return context.getString(R.string.import_invalid_message, detail)
    }
    ...
    /**
     * Derive a stable, filesystem-safe hack id from [suggestedName] (the patch
     * filename without extension). Ensures uniqueness against existing user hacks
