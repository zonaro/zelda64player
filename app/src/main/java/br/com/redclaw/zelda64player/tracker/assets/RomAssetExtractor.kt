...
class RomAssetExtractor(
    ...
    suspend fun extractAll(baseRomFile: File, game: TrackerGame): Result<ExtractReport> =
        withContext(Dispatchers.IO) {
            runCatching {
                require(baseRomFile.exists()) { "Base ROM not found: ${baseRomFile.absolutePath}" }

                val crc32 = ChecksumCalculator.crc32(baseRomFile)
                val mappings = when (game) {
                ...
                val dmaOffset = DmaTableOffsets.forRom(header.gameCode, header.versionByte)
                    ?: return@runCatching ExtractReport(
                        extracted = 0,
                        failed = mappings.size,
                        errors = listOf("ROM ${header.gameCode} v${header.versionByte} not supported for extraction"),
                    )

                val parser = DmaTableParser(baseRomFile, dmaOffset)
                val entries = parser.parseEntries()
                val iconIndex = when (game) {
                    TrackerGame.OOT -> DmaTableOffsets.OOT_ICON_FILE_INDEX
                    TrackerGame.MM -> DmaTableOffsets.MM_ICON_FILE_INDEX
                }
                val iconEntry = entries.getOrNull(iconIndex)
                    ?: return@runCatching ExtractReport(
                        extracted = 0,
                        failed = mappings.size,
                        errors = listOf("DMA entry $iconIndex not found (table size ${entries.size})"),
                    )
                ...
                val archiveBytes = if (iconEntry.isCompressed) {
                ...
                for (mapping in mappings) {
                    try {
                        val pixels = when (mapping.format) {
                            br.com.redclaw.zelda64player.tracker.assets.graphics.N64TextureFormat.RGBA16 ->
                                TextureDecoder.decodeRGBA16(archiveBytes, mapping.offset, mapping.width, mapping.height)
                            br.com.redclaw.zelda64player.tracker.assets.graphics.N64TextureFormat.CI8 -> {
                                val tlutOff = mapping.tlutOffset
                                    ?: error("CI8 mapping ${mapping.itemId} missing tlutOffset")
                                TextureDecoder.decodeCI8(archiveBytes, mapping.offset, mapping.width, mapping.height, archiveBytes, tlutOff)
                            }
                            else -> error("Unsupported format ${mapping.format} for ${mapping.itemId}")
                        }
