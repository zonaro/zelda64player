...
class MultiStoreCatalogTest {

    @Test
    fun picksParserStampsStoreId() {
        val json = JSONObject().apply {
            put("storeName", "Zelda 64 Picks")
        ...
        assertEquals("Zelda 64 Picks", parser.storeName(json))
    }
    ...
    fun hackEntryDownloadTargetRoundTrips() {
        ...
        assertEquals("hylianmodding", restored.storeId)
        assertEquals("mods", restored.sourceCatalogId)
    ...
    fun hackEntryDirectPatchTargetRoundTrips() {
        ...
        val original = HackEntry(
            id = "hm_y", name = "Y", description = "d", author = "a", version = "1.0",
            baseRom = br.com.redclaw.zelda64player.data.model.BaseRomRef(
                "MM", "NSME", -1,
                br.com.redclaw.zelda64player.data.model.Checksums("", null, null)
            ),
